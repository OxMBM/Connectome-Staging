function braidsurfaceplot2(braid_ind,dam)
%  ---------------------------------------------------
%
...
for i = 1:nd
    P1(:,i) = dam(i)*resprod1(:,:,i)';
    P2(:,i) = dam(i)*resprod2(:,:,i)';
    P3(:,i) = dam(i)*resprod3(:,:,i)';
    P4(:,i) = dam(i)*resprod4(:,:,i)';
    P5(:,i) = dam(i)*resprod5(:,:,i)';
    P6(:,i) = dam(i)*resprod6(:,:,i)';
    P7(:,i) = dam(i)*resprod7(:,:,i)';
    P8(:,i) = dam(i)*resprod8(:,:,i)';
    P9(:,i) = dam(i)*resprod9(:,:,i)';
    P10(:,i) = dam(i)*resprod10(:,:,i)';
    P11(:,i) = dam(i)*resprod11(:,:,i)';
    P12(:,i) = dam(i)*resprod12(:,:,i)';
    P13(:,i) = dam(i)*resprod13(:,:,i)';
    
    P14(:,i) = dam(i)*resprod14(:,:,i)';
    P15(:,i) = dam(i)*resprod15(:,:,i)';
    P16(:,i) = dam(i)*resprod16(:,:,i)';
    P17(:,i) = dam(i)*resprod17(:,:,i)';
end
B = [1:99]';
...
xlim([-1 101])
ylim([min(log(dam))-0.3 max(log(dam))+0.3])
xlabel('biomarker abnormality [%]','fontweight','bold')
ylabel('log(\delta)','fontweight','bold')
