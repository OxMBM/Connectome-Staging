function [braid_val,braid_ind,braid_conc] = damagebraidsurface_difvar(L,u0,n,TIME,dam,dif)
%  ---------------------------------------------------
%
% Oxford Mathematical Brain Modelling Group
%   Staging and model selection using braid surfaces for    
%   a damage associated with Fisher-Kolmogorov-Petrovsky-Piskunov model
%
...
tic
for i = 1:nd
[T,Y] = ode45(@fkppxdam,TIME,[u0' zeros(1,n)],[],dam,dif(i),L,n);
j = 1;
        area1 = [Y(:,n+(region(j,1):region(j,2))) Y(:,n+(region(j+nreg,1):region(j+nreg,2))) Y(:,n+(region(j+1,1):region(j+1,2))) Y(:,n+(region(j+1+nreg,1):region(j+1+nreg,2)))];
        MEAN_REGION(j,:) = mean(area1');
        MEAN_REGION(2,:) = NaN;
j = 3;
        area2 = [Y(:,n+(region(j,1):region(j,2))) Y(:,n+(region(j+nreg,1):region(j+nreg,2))) ...
            Y(:,n+(region(j+1,1):region(j+1,2))) Y(:,n+(region(j+1+nreg,1):region(j+1+nreg,2))) ...
            Y(:,n+(region(j+2,1):region(j+2,2))) Y(:,n+(region(j+2+nreg,1):region(j+2+nreg,2))) ...
            Y(:,n+(region(j+3,1):region(j+3,2))) Y(:,n+(region(j+3+nreg,1):region(j+3+nreg,2)))];
        MEAN_REGION(j,:) = mean(area2');
        MEAN_REGION(4,:) = NaN;
        MEAN_REGION(5,:) = NaN;
        MEAN_REGION(6,:) = NaN;
j = 7;
        MEAN_REGION(j,:) = mean([Y(:,n+(region(j,1):region(j,2))) Y(:,n+(region(j+nreg,1):region(j+nreg,2)))]');
        MEAN_REGION(j+1,:) = mean([Y(:,n+(region(j+1,1):region(j+1,2))) Y(:,n+(region(j+1+nreg,1):region(j+1+nreg,2)))]');
        MEAN_REGION(j+2,:) = mean([Y(:,n+(region(j+2,1):region(j+2,2))) Y(:,n+(region(j+2+nreg,1):region(j+2+nreg,2)))]');
